...
\newcommand{\apo}{{\textrm{'}}}  % apostrophe
...
\hline
MP &  \rule[-2.5ex]{0pt}{7ex} $\frac{A\to B\quad A}{B}$ & - \\ \hline %(\textrm{MP})
\hline
%\hline
%\infer[(\forall\textrm{I})]{A\to \forall x B}{A\to B}, если $x \notin \textrm{FV}(A)$\\
\rule[-2.5ex]{0pt}{7ex}
$\forall$I & $\frac{A\to B}{A\to \forall x B}$ & $x \notin \textrm{FV}(A)$\\ \hline %(\forall\textrm{I})
\hline
%\hline
\rule[-2.5ex]{0pt}{7ex}
$\exists$E & $\frac{A\to B}{\exists x A\to B}$ & $x \notin \textrm{FV}(B)$\\ %(\textrm{$\exists$E})
\hline
\end{tabular}
\end{table}
...
\\2) $x = A. \Longrightarrow \Tr(x)$ (by H1) % assumption, part 1)
\\\myqed
\end{theorem}
