...
\item $\forall X,Y,Z \in \Ob_C.\forall f\in\Hom_C(Y,Z).\forall g\in\Hom_C(X,Y). \widetilde{F}^{X,Z}(f\circ_C^{X,Y,Z} g)=\widetilde{F}(f)\circ_D^{F(X),F(Y),F(Z)} \widetilde{F}(g)$
\end{enumerate}
...
$D \coloneqq \Set$, 
$F \coloneqq \Hom_C(-,x)$, 
$\widetilde{F} \coloneqq \HomMor_C(-,x)$\\
\begin{enumerate}
...
Let's prove that $\HomMor_C(f\circ_C g, x) = \HomMor_C(g, x) \circ_\Set \HomMor_C(f, x)$\\
